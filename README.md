...
<details> <summary>트러블슈팅 2 서버다운  </summary>  
	
### **ISSUE**
	
- 초반 서버 접속 시, 페이지 이동 10번 쯤 진행하게 되면 갑자기 서버가 다운되는 이슈가 있었다.  다른 부분을 작업중인데다, 그만큼 이동할 일이 잦지 않아 발견이 늦었다.  
- 테스트 후 아무 페이지 이동이 아닌 헤더의 게시판 이동 메뉴 *10 시 발생하는 것을 알 수 있었는데, 코드를 살펴보니 게시글 데이터를 가져오기 위해 db 연결 후 connection close를 누락한 것.  (종료되지 않은 connection pool이 점점 중첩되어 maxactive(10이었다)를 초과한 것이었다.)
...
1. resource/static 아래에 정적 폴더로 이미지를 넣고 출력  
	서버 구동 시 읽혀지고 정적자원을 사용할 수 있게 올라간다.  
	이 이슈 외에도 실제 배포 시에 파일이 삭제될 가능성이 크다고 한다.  
    
2. 외부 폴더를 추가하여 실제 시스템의 폴더 경로를 이용하는 방법 (채택)  
...
4. 외에도 정적폴더를 사용하면서, 서버를 자동으로 재시작하게 하거나, 정적 리소스 요청에 대해 기본 경로를 설정해주는 방법이 있는 것 같았다.  
